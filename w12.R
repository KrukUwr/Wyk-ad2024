...
# system.time({ # 1100 sec. (na 5 wątkach)
#   set.seed(123)
# 
#   cl <- makeCluster(5)
#   registerDoSNOW(cl)
...
# system.time({ # 3400 sec. (na 5 wątkach)
#   set.seed(123)
# 
#   cl <- makeCluster(5)
#   registerDoSNOW(cl)
...
svm7v <- svm(IfSigned ~ NoCalls + NoPay + IfReach + PrcVisitsCl + 
    IfMan + D_ContractDateToImportDate + TOA, 
  data=casesTrn, kernel="radial", cost=10, scale=TRUE)
...
# system.time({ # 930 sec.
#  set.seed(123)
#  svmTune <- tune(svm, train.x=casesTrn[, .SD, .SDcols=c("NoCalls", "NoPay",
#   "IfReach", "PrcVisitsCl", "IfMan", "D_ContractDateToImportDate", "TOA")],
...
svm7vTuned <- svm(IfSigned ~ NoCalls + NoPay + IfReach + PrcVisitsCl + 
    IfMan + D_ContractDateToImportDate + TOA, 
  data=casesTrn, kernel="radial", cost=50, gamma=0.05, scale=TRUE)
...
frmCl <- as.formula("IfSigned + IfNotSigned ~ NoCalls + NoPay + IfReach + 
  PrcVisitsCl + IfMan + D_ContractDateToImportDate + TOA")
...
nnetFitClPred <- compute(nnetFitCl, casesTstStd[, .SD, .SDcols=c(
  "NoCalls", "NoPay", "IfReach", "PrcVisitsCl", "IfMan", 
  "D_ContractDateToImportDate", "TOA")])
...
frm <- as.formula("IfSigned ~ NoCalls + NoPay + IfReach + 
  PrcVisitsCl + IfMan + D_ContractDateToImportDate + TOA")
...
#  set.seed(123)
#  nnetFit <- neuralnet(frmAll, data=casesTrnStd, hidden=c(6, 5, 4, 3),
#    stepmax=2e6, threshold = 0.1)
# })
# 
...
load("nnet653all.RData") # not converged
...
# Uwaga: Keras jest narzędziem napisanym w języku Python, 
...
# Definicja modelu
nnKeras <- keras_model_sequential(input_shape=c(21)) 
nnKeras %>% 
...
# Definicja modelu B
nnKerasB <- keras_model_sequential(input_shape=c(21))
nnKerasB %>%
  layer_dense(units=18, activation='relu',
    kernel_initializer = "he_uniform") %>%
  layer_dropout(rate=0.3) %>%
...
# Zapisz model
nnKerasB %>% save_model("nnKerasB.keras")

# Wczytaj model
nnKerasB <- load_model("nnKerasB.keras")
summary(nnKerasB)
