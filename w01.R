...
dTable[, X:=sample(c("G", "C", "H"), nrow(dTable), replace=TRUE)] # znakowa
dTable[, Y:=rnorm(nrow(dTable))] # liczbowa
dTable[, `:=`(
  Coin=sample(c("O" ,"R"), nrow(dTable), replace=TRUE),
...
# są też operacje z rodziny grouping sets
rollup(dTable, 
.(.N,
  AVGsLength=mean(sLength)), 
by=c("Coin", "Cube")) 
...
sets=list(
  c("Coin", "Cube"),
  c("Coin"),
  character(0))) # character(0) dla grand total/bez grupowania

# Uwaga: W funkcjach rollup, cube, groupingsets argument 'by' wymaga wektora 
# stringóœ (nazw kolumn, po których chcemy grupować).
