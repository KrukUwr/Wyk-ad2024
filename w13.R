...
# tab3000 <- as.data.table(sortedGrid@summary_table)[, c("auc",
#   "col_sample_rate", "col_sample_rate_per_tree",
#   "histogram_type", "learn_rate", "max_depth",
#   "min_rows", "nbins", "sample_rate")]
# save(list=c("sortedGrid"), file="grid3000.RData")
# save(list=c("tab3000"), file="gridTab3000.RData")
load("grid3000.RData")
load("gridTab3000.RData")

sortedGrid
tab3000

# Jaki procent modeli przejrzeliśmy?
tab3000[, .N]/prod(unlist(lapply(gbmPar, length)))

# Czy dostrzegamy jakieś prawidłości?
dev.new()
par(mfrow=c(3, 3))
for (i in 2:9) {
  plot(as.factor(tab3000[[i]]), tab3000$auc, 
    ylab="AUC", xlab=names(tab3000)[i])
}
...
x <- names(tab3000)[2] 
y <- names(tab3000)[6]
plot3d(as.numeric(tab3000[[x]]), as.numeric(tab3000[[y]]), tab3000$auc, 
  xlab=x, ylab=y, zlab="AUC")

tmp <- tab3000[, .(auc=median(as.numeric(auc))), by=c(x, y)]
